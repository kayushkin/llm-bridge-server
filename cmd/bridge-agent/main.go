...
func main() {
	...
	//
	// What the run is *about* belongs in the prompt, which already becomes the
	// session's display name. Purpose says what kind of session it is, and for
	// anything bridge-agent starts that is "delegate".
	if _, known := msg.LookupPurpose(*purpose); !known {
		fatal("unknown purpose %q — purpose is a category, not a description of the task.\n"+
			"Run bridge-agent -list-purposes to see the registered slugs, or leave the flag unset for %q.\n"+
			"To add a new one, add it to msg/purpose.go in the llm-bridge repo.",
			*purpose, msg.PurposeDelegate)
	}
