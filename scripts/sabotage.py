...
def score(target: Path, packages, cases, guard_markers=()):
    ...
    The check above this one — `git status` on the target — is a different guard. It
    stops this harness deleting somebody's uncommitted work. It cannot see a concurrent
    run at all, because the other run restores each file before the next case and the
    tree is clean between mutations exactly when it is most dangerous to trust.
    """
