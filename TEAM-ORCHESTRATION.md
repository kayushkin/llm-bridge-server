...
A session attribute records the difference: **`controlled_by` ∈ {`bridge`, `harness`}** (derivable
...
- **`type`** (interactive / autonomous / system) · **`purpose`** (chat / subagent / conformance …) ·
  **`role`** (builder / reviewer / …) · **`controlled_by`** (`bridge` / `harness` — who can message /
  steer / kill this session; frontends disable direct actions when `harness`, §12) — classification labels.
...
| **Internal subagent (L2)** — spawned inside the harness | a session, **adapter-promoted** | own `bridge_session_id` + `manager_session_id`, `controlled_by=harness` | nested under its manager like any subagent, badged "internal"; visible/attributable but **not independently killable** (§12) |

Nesting keys on `manager_session_id` — uniform for L1 and L2. `role`/`type`/`purpose`/`controlled_by`
...
1. Add `parent_tool_use_id` to `ccStreamEvent` (`translate.go:15`); surface it on canonical `msg.Event`
   as **`harness_parent_id`** (`HarnessParentID`, joining the `harness_session_id`/`harness_message_id`
   family). *Landed additively in `llm-bridge` — along with `manager_session_id` / `root_session_id` /
   `depth` / `controlled_by` / `refreshed_from_session_id` on `ManagedSession`.*
2. **Mint + map** — the first non-null `parent_tool_use_id` (or the `task_started` for that
   ...
   `manager_session_id` = the parent's `bridge_session_id`, `controlled_by = harness`. Use the
...
### 21.6 Subagent identity hazards — the "MCP resume id error" incident

A live incident (session "MCP resume id error") exposed two failure modes, both rooted in subagents not
being clearly marked. **The naive fix makes it worse** — recorded here so it isn't re-attempted.

1. **Non-resumable id in the resume slot.** Claude Code names each subagent rollout
   `…/<parent-uuid>/subagents/agent-<hex>.jsonl`. `llm-bridge-claudecode/discover.go` stores that
   `agent-<hex>` in `harness_session_id` — but it's an internal agent id, **not a resumable Claude
   UUID**. Anything that runs `claude -p --resume <id>` (the renamer auto-titler; dispatcher/reviewer
   nudges) then fails: *"--resume requires a valid session ID…"*. Observed at scale: **810 `agent-*`
   rows** in `bridge.db`, each a latent `--resume` failure. Violates the harness session-id contract
   (no-native-resumable-id ⇒ `harness_session_id` must be **empty**, never a stand-in).

2. **`harness_session_id` is overloaded — do NOT just blank it.** It's *also* the discovery **de-dup
   key** (`UpsertDiscoveredSession` falls back to it because CC reports no `bridge_session_id` for
   *internal* subagents). With **810 distinct `agent-<hex>` keys and 483 already-empty rows**, blanking
   would make the dedup `WHERE harness_session_id=''` collapse subagents onto each other / unrelated
   empty-harness rows — and a same-tick re-discovery would mint duplicates. The clean fix must first
   **move subagent de-dup off `harness_session_id`** (onto the rollout path or a dedicated subagent id),
   *then* leave `harness_session_id` empty. A bulk backfill is unsafe until that lands.

**The durable fix = the marking.** Every subagent carries `purpose=subagent` + the new
`controlled_by=harness`. **All resume / message / kill callers gate on `controlled_by==bridge`** — never
act on a harness-controlled session. That fixes the `--resume` bug *without touching the de-dup key*,
and it's the same marker the dash UI gates on (§12). The contract cleanup (empty `harness_session_id` +
dedup on a non-resume key, plus the 810-row backfill) follows as a separate import-path change.
