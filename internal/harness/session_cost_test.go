...
func TestSessionCost_TakesTheLargerLowerBoundPerProcess(t *testing.T) {
	...
	deriveWithoutStatus(d, apiCall(0.30))
	cost := lastSessionCost(t, deriveWithoutStatus(d, apiCall(0.20)))
	...
	// lost): results lead.
	cost = lastSessionCost(t, deriveWithoutStatus(d, pricedResult(0.80)))
	...
	cost = lastSessionCost(t, deriveWithoutStatus(d, apiCall(0.25)))
	...
	cost = lastSessionCost(t, deriveWithoutStatus(d, apiCall(0.40)))
...
func TestSessionCost_ANewProcessStartsFromTheRecordedCost(t *testing.T) {
	...
	cost := lastSessionCost(t, deriveWithoutStatus(d, apiCall(0.10)))
	...
	cost = lastSessionCost(t, deriveWithoutStatus(d, pricedResult(0.05)))
	if !near(cost.TotalUSD, 3.70) {
...
func TestSessionCost_AnUnpricedResultEmitsNothing(t *testing.T) {
	d := newDerivationState()
	for _, ev := range deriveWithoutStatus(d, &msg.Event{Type: msg.EventResult, Result: &msg.ResultEvent{Text: "ok"}}) {
		if ev.Type == msg.EventSessionCost {
