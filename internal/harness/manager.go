...
func (m *Manager) readEvents(proc HarnessProcess) {
	...
	// Process exited — close all subscriber channels. Subagent sessions live
	// and die with the process that hosts them, so they are torn down here
	// too: their per-session maps would otherwise leak for the lifetime of the
	// server, and any subagent still marked running (the process died before
	// its task reported a terminal status) would sit at "running" forever.
	m.mu.Lock()
